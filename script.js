...
function createSquares(n) {
  ...
  for (let i = 0; i < height; i++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      square.style.cssText += `height:${window.innerWidth / n}px; width: ${
        window.innerWidth / n
      }px`;
...
function paint(target) {
  document
    .querySelector(`[data-counter="${target}"]`)
...
function shade(target) {
  document
    .querySelector(`[data-counter="${target}"]`)
    .classList.add(...["selected", "shaded"]);
}

function errase(target) {
  document
    .querySelector(`[data-counter="${target}"]`)
    .classList.remove(...["selected", "shaded"]);
}
